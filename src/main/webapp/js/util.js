...
function isLogin(){
 var check="";
	$.ajax({
		url:"/jclq/query/isLogin.jsp",//后台处理程序
		type:"POST",//数据发送方式
		async:false, 
		dataType:'html',//接受数据格式
		success:function(data){
...
function ajaxLoginOut(){
	$.ajax({
	    type: "POST",
	    url: "/jclq/loginOut.do",
	    success: function(msg){
	    	window.location.href="http://users.ruyicai.com/login.jsp";
			topLogin();
		  indexRight();
	     }
	   });
...
//成功页面显示相应的彩种以及图片
  function getCaiZhong(divname1,divname2){
		  ...
		  switch(key){
		  case "J00005":
			  div2.html('<a href="/jclq/touzhu/shengfu.jsp"  title="'+decodeURI(EncodeUtf8('继续投注'))+'" >'+decodeURI(EncodeUtf8('继续投注')+'</a>'));
			  break;
		  case "J00006":
			  div2.html('<a href="/jclq/touzhu/rfshengfu.jsp"  title="'+decodeURI(EncodeUtf8('继续投注'))+'" >'+decodeURI(EncodeUtf8('继续投注')+'</a>'));
			  break;
		  case "J00007":
			  div2.html('<a href="/jclq/touzhu/shengfencha.jsp"  title="'+decodeURI(EncodeUtf8('继续投注'))+'" >'+decodeURI(EncodeUtf8('继续投注')+'</a>'));
			  break;
		  case "J00008":
			  div2.html('<a href="/jclq/touzhu/daxiaofen.jsp"  title="'+decodeURI(EncodeUtf8('继续投注'))+'" >'+decodeURI(EncodeUtf8('继续投注')+'</a>'));
			  break;
		  default:
		  	  div2.html('<a href="/jclq/touzhu/shengfu.jsp" title="'+decodeURI(EncodeUtf8('继续投注'))+'" >'+decodeURI(EncodeUtf8('继续投注')+'</a>'));  
		  	  break;
...
function newBaseTab(t) {
	var ControlTarget = t.attr("ControlTarget");
	t.siblings().removeClass("selected").end().addClass("selected");
	$(ControlTarget).siblings().removeClass("selected").end().addClass("selected");
	$("#showClass").val(ControlTarget);
};
...
function change(classname){
	 $(".day"+classname).toggle();
	 $(".hide"+classname).toggle();
	 $(".show"+classname).toggle();
}
function changtime(){
$(".end_Times").toggle();
$(".game_Times").toggle();
}
function hideTeam(tdId){
	$("#"+tdId).toggle();
	$("#"+tdId).next().toggle();
	//显示截至场数和隐藏场数
	var teamNum = $("#hidTemNum").html();
	teamNum= parseInt(teamNum)+1;
	$("#hidTemNum").html(teamNum);
	/*var JZTemNum = $("#JZTemNum").html();
	JZTemNum= parseInt(JZTemNum)+1;
	$("#JZTemNum").html(JZTemNum);*/
	//保存隐藏的赛事编号
	var teamNum = $("#hidTemId").val();
	$("#hidTemId").val(teamNum+tdId+",");
}
function recoveryTem(){
	var teamNum = $("#hidTemId").val();
	var num = teamNum.split(",");
	for (var i = 0; i < num.length-1; i++) {
		$("#"+num[i]).show();
		$("#check_"+num[i]).attr("checked","checked");
	}
	$("#hidTemNum").html("0");
	//$("#JZTemNum").html(JZTemNum);
}
...
function getObjForNameAndValueToKey() {
	if (isLogin()) { //判断session是否失效 如果失效跳转到登陆页面
		var flowno = "flowno=" + GetQueryString("flowno");
		var play_name = "play_name=" + GetQueryString("play_name");
		var batchcode = "batchcode=" + GetQueryString("batchcode");
		...
		$.ajax({
			url:"/jclq/getBetSelect.do",//后台处理程序
			type:"POST",//数据发送方式
			data:parameters,//参数
			dataType:'html',//接受数据格式
			success:function(data){
