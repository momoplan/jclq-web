...
public class JSONReslutUtil {
	...
	public static JSONObject getUserNo(HttpServletRequest request){
		...
		if(jsessionid != null){
			try {
				return JSONObject.fromObject(getResultMessage(usersCenterUrl+"user/center!getUserNo;jsessionid="+jsessionid+"?","a="+Math.random(),"POST"));
			} catch (IOException e) {
	...
	public static JSONObject getUserInfo(HttpServletRequest request){
		...
		if(jsessionid != null){
			try {
				return JSONObject.fromObject(getResultMessage(usersCenterUrl+"user/center!getUserInfo;jsessionid="+jsessionid+"?","a="+Math.random(),"POST"));
			} catch (IOException e) {
				logger.info("登录信息查询出现异常");
			}
		}
