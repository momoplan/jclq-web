    ...
    pageEncoding="UTF-8"%>
		...
		<div class="competingEventLeft">
		  ...
		  <table class="competingEventTable" >
			...
			<option value="1">截止时间</option>
				...
				<td width="177" rowspan="2">客队  让分  主队</td>
				<!-- <td width="102" rowspan="2">
				<select name="">
			<option>比赛时间</option>
			</select></td> -->
			 ...
			 <c:forEach items="${duizhenInfo}" var="duizhens"  >
			   ...
			   <tr class="show${duizhen.day}" style="display:none">
				<th colspan="7" class="week">${duizhen.newday}${duizhen.newweek}　<font onclick='change("${duizhen.day}")' >显示</font></th>
			  </tr>
			   <tr class="hide${duizhen.day}">
				<th colspan="7" class="week">${duizhen.newday}${duizhen.newweek}　<font onclick='change("${duizhen.day}")' >隐藏</font></th>
			  </tr>
			   </c:if>
			  <tr <c:if test="${s.index%2!=0}">class="gray day${duizhen.day}"</c:if>  class="day${duizhen.day}" id="${duizhen.teamid}">
				...
				<td><font id="v_${duizhen.day}_${duizhen.teamid}">${duizhen.team1}</font>  ${duizhen.peilv.letVs.basePoint }  <font id="h_${duizhen.day}_${duizhen.teamid}">${duizhen.team2 }</font></td>
				<!-- <td>1.49　3.75</td> -->
				<th><span class="result_3 result_${duizhen.teamid}"  id="result_1_${duizhen.day}_${duizhen.teamid}" onclick='checkColor($(this),"${duizhen.day}","${duizhen.newweek}",1,"${duizhen.teamid}")'>${duizhen.peilv.letVs.v0 }</span></th>
				<th><span class="result_3 result_${duizhen.teamid}" id="result_0_${duizhen.day}_${duizhen.teamid}"  onclick='checkColor($(this),"${duizhen.day}","${duizhen.newweek}",0,"${duizhen.teamid}")'>${duizhen.peilv.letVs.v3 }</span></th>
			  </tr>
			  </c:forEach>
			  </c:forEach>
			  </tbody>
	  ...
	  */
	  /*function checkColor(obj,day,week,selected,teamid){
		  ...
		  $("#matchNum").html(rfmatchNum);
	  }
	//显示自由过关咯多串过关的选项
	  function addRighBWInfo(obj,day,week,daxiao,teamid){
		  if(rfmatchNum>1&&rfmatchNum<9){
			  $("#r"+rfmatchNum+"c1").css("display","");
		  }
		//定义数组存放多串投注方式时的id编号
			if (rfmatchNum == 3) {
				var more = ["3", "4"];
			} else if (rfmatchNum == 4) {
				var more = ["4", "5", "6", "11"];
			} else if (rfmatchNum == 5) {
				var more = ["5", "6", "10", "16", "20", "26"];
			} else if (rfmatchNum == 6) {
				var more = ["6", "7", "15", "20", "22", "35", "42", "50", "57"];
			} else if (rfmatchNum == 7) {
				var more = ["7", "8", "21", "35", "120"];
			} else if (rfmatchNum == 8) {
				var more = ["8", "9", "28", "56", "70", "247"];
			}

			for (var j = 3; j <= rfmatchNum; j++) {
				for (var m = 0; m < more.length; m++) {
					var id = "r" + rfmatchNum + "c" + more[m];
					$("#" + id).css("display", "");
				}
			}
	  ...
	  //显示自由过关咯多串过关的选项
	  function removeRighBWInfo(rfmatchNum){
				  $("#r"+rfmatchNum+"c1").remove();
				  //定义数组存放多串投注方式时的id编号
					if (rfmatchNum == 3) {
						var more = ["3", "4"];
					} else if (rfmatchNum == 4) {
	  ...
	  function addRighTZInfo(obj,day,week,selected,teamid){
		  ...
		  var info ="<tr id='choose_"+day+"_"+teamid+"'><td><dl onclick='removeInfo($(this),"+day+","+selected+","+teamid+",1)' class='CheckBox light Switch'><dt></dt><dd id='no-"+day+"-"+teamid+"'>"+week+teamid+"</dd></dl></td><td id='homeTeam-"+day+"-"+teamid+"'>"+hTeam+"</td><td><span onclick='removeInfo($(this),"+day+",1,"+teamid+",0)' class='CheckWin' id='da-"+day+"-"+teamid+"'>主负</span><span onclick='removeInfo($(this),"+day+",0,"+teamid+",0)'class='CheckLoss' id='xiao-"+day+"-"+teamid+"'>主胜</span></td><td><input name='' type='checkbox' value='' /></td></tr>";
